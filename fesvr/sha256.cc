...
#include <fstream>
#include <vector>
bool SHA256::sha256file(const char *name, size_t *charCount, unsigned char hashResult[32])
{
  ...
  std::vector<char> rdbuf(4096, 0);
  SHA256 hash;
  size_t count = 0;

  while (!fin.eof())
  {
    fin.read(rdbuf.data(), rdbuf.size());
    std::streamsize s = fin.gcount();
    hash.add(rdbuf.data(), s);
    count += s;
  }
  if (charCount)
    *charCount = count;
...
std::string SHA256::bytesHashToStringHash(const unsigned char sha256[32])
{
  char buf[32 * 2 + 1];
  size_t i;
  for (i = 0; i < 32; i++)
  {
    char h = (sha256[i] & 0xf0) >> 4;
    char l = (sha256[i] & 0xf);
    buf[i * 2] = h < 0xa ? '0' + h : 'a' + h - 0xa;
    buf[i * 2 + 1] = l < 0xa ? '0' + l : 'a' + l - 0xa;
  }
  buf[i * 2] = '\0';
  return buf;
}
